...
# 3. PCA evaluation on 2007 RMA data 
# 4. PCA regression using 2007 data 
# 
...
# --------------------------------------------------------------------------
#  c("err_2008_bpca" = rec_err_vbpca, "err_2008_spca" = rec_err_spca)
...
# --------------------------------------------------------------------------
# Calculate reconstruction error for Bayes PCA 
# --------------------------------------------------------------------------
# Wmat <- bpca_mod[[1]]
# Pmat <- bpca_mod[[2]]
# ind_zeros <- which(bpca_mod[[9]] <= 0.5)
...
# rec_X_2007_bpca <- as.matrix(X_2007) %*% Wmat %*% t(Pmat)
# rec_err_X_2007_bpca <- sum((X_2007 - rec_X_2007_bpca)^2)

# --------------------------------------------------------------------------
# Calculate reconstruction error for Sparse PCA
# --------------------------------------------------------------------------
# Wmat_spca <- mod$loadings
# sVd <- svd(XTX %*% mod$loadings)
# Pmat_spca  <- sVd$u %*% sVd$v		

# rec_X_2007_spca <- as.matrix(X_2007) %*% Wmat_spca %*% t(Pmat_spca)
# rec_err_X_2007_spca <- sum((X_2007 - rec_X_2007_spca)^2)

# --------------------------------------------------------------------------
# bpca vs spca Reconstruction errors 
# --------------------------------------------------------------------------
# c("err_2007_bpca" =  rec_err_X_2007_bpca, "err_2007_spca" = rec_err_X_2007_spca)

###################################
#	4. PCA regression - TIV 2007  #
###################################
# --------------------------------------------------------------------------
